...
import { login, modelDeleteTag, Conn } from "../api";

export default class Modeldeletetag extends Command {
  static description = "Delete tag from model";
  ...
  async run() {
    ...
    try {
      ...
      var docs = await modelDeleteTag(this.conn, args.project, args.model, args.tag);
      console.log(docs);
    } catch (error) {
      throw error;
    }
