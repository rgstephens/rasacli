...
import { login, modelDeleteTag, printFlagsArgs, Conn } from "../api";

export default class Modeldeletetag extends Command {
  static description = "Delete tag from model, enterprise vesion only";
  ...
  async run() {
    const { args, flags } = this.parse(Modeldeletetag);
    this.conn = {
    ...
    try {
      ...
      var docs = await modelDeleteTag(this.conn, flags.project, flags.model, flags.tag);
      console.log(docs);
    } catch (error) {
      throw error;
    }
